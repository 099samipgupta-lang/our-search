...
def main():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(COUNT):
            ...
            if activator.activate(hostname):
                activated += 1
        ...
        print(
            f"CRAWLER_ADD_URL_CALLS: "
            f"{len(crawler.added_urls)}"
        )
        ...
        reopened_domains = DomainCandidateStore(
            domain_db
        )

        reopened_expansion = ExpansionCandidateStore(
            expansion_db
        )

        reopened_queue = ExpansionQueue(
            expansion_db
        )
